...
## Acceptance criteria
- [x] Move all stories from `src/stories/` to their respective widget folders (e.g. `src/stories/ActivityBar.stories.tsx` moves to `src/components/widgets/`).
- [x] Update Storybook configuration file to scan for stories at `../src/**/*.stories.@(js|jsx|mjs|ts|tsx)`.
- [x] Run `npm run build-storybook` successfully to ensure the paths resolve correctly.
